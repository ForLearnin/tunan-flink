...
object FlinkKafkaIO {
    ...
    private def kafkaConsumer(env: StreamExecutionEnvironment) = {
        val properties = new Properties
        properties.put("bootstrap.servers", BOOTSTRAP_SERVERS)
        properties.setProperty("group.id", GROUP_ID)

        env.setStreamTimeCharacteristic(TimeCharacteristic.EventTime)
        env.addSource(new FlinkKafkaConsumer[String](TOPIC, new SimpleStringSchema(), properties)).setParallelism(2).filter(x => x.trim.nonEmpty)
