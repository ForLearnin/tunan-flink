...
class StudentStream extends RichParallelSourceFunction[Student]{

    var SQL = "select * from study.student order by id desc"
    ...
    override def run(ctx: SourceFunction.SourceContext[Student]): Unit = {
        ...
        while(rs.next()){
            ...
            ctx.collect(Student(id,name,age,sex,school,time))
        }
    }
